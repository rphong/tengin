...
Tank::Tank(VAO&& vao, const glm::vec2& pos, const float& angle, const float& speed)
    : m_vao(std::move(vao)), m_pos(pos), m_tankAngle(angle), m_barrelAngle(angle), m_speed(speed) {}
...
void Tank::draw(const Shader& shader) const {
    ...
    model = glm::rotate(model, glm::radians(m_tankAngle), glm::vec3(0.0f, 0.0f, 1.0f));
