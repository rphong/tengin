...
void TextRenderer::loadCharacters() {
    ...
    if (!validInitialLoad(ft, face)) return;
...
bool TextRenderer::validInitialLoad(FT_Library& ft, FT_Face& face) {
    if (FT_Init_FreeType(&ft)) {
        std::cout << "ERROR::FREETYPE: Could not init FreeType Library"
                  << std::endl;
        return false;
    }
    if (FT_New_Face(ft, "../tengin/src/resources/fonts/Roboto-Medium.ttf", 0,
                    &face)) {
